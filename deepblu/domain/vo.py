...
TPassword = TypeVar("TPassword", bound="PasswordStr")


class PasswordStr(ConstrainedStr, Primitive):
    """
    Password primitive is a string with a minimum length of 8 characters.
    ...
    isinstance(user.password, PasswordStr) # True
    isinstance(user.password, str) # True

    hashed = PasswordStr.hash("1234567") # raises ValidationError
    hashed = PasswordStr.hash("12345678")
    # $2b$12$R1Uej9/bZSO/VCQ0e9PR6etdreVaDQqBqVA0rvuKiBwfAuxQG/9.q

    PasswordStr("12345678").compare(hashed) # True
    PasswordStr("abcdefghi").compare(hashed) # False
    PasswordStr.verify("12345678", hashed) # True
    PasswordStr.verify("abcdefghi", hashed) # False
    ...
    def compare(self, hashed: str) -> bool:
        return PasswordStr.verify(self, hashed)
